        ...
        function page:site-content-if-blog-db-does-not-exist()
{
  ...
  return
   <html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
      <div class="right"><img src="/simple-basex-webapp/basex.svg" width="96"/></div>
      <h2>{ $title }</h2>

      <p>You have to <a href="/simple-basex-webapp/restxq/blog-v1/create-database">create the blog database</a> first.</p>
    </body>
  </html>
};
  ...
  return
   <html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
      ...
      <p>Have a look at the <a href="/simple-basex-webapp/restxq/blog-v1/show-blog-xml">xml source</a> of the blog.</p><br />

      <!-- pathes relative to root -->
      <p><a href="/simple-basex-webapp/restxq/blog-v1/drop-database">Drop blog database</a></p>
    </body>
  </html>
};
...
declare %restxq:path("blog-v1/create-database")
  ...
  let $prop := proc:property('java.runtime.version')
  let $trace := trace($prop, "Java version-->")

  let $prop := proc:property('org.basex.host')
  let $trace := trace($prop, "org.basex.host-->")

  let $prop := proc:property('org.basex.port')
  let $trace := trace($prop, "org.basex.port-->")

  let $prop := proc:property('org.basex.dbpath')
  let $trace := trace($prop, "org.basex.dbpath-->")
